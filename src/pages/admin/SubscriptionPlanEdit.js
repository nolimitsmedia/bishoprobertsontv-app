// src/pages/admin/SubscriptionPlanEdit.js
import React from "react";
import { useNavigate, useParams } from "react-router-dom";
import api from "../../api";
import SubscriptionPlanNew from "./SubscriptionPlanNew";

/**
 * This file reuses the PlanEditor embedded in SubscriptionPlanNew.js.
 * To avoid duplication, we simply import the default and copy the component code here
 * would be heavy. Instead, we load the same code again by exporting it from New page.
 *
 * But since PlanEditor is local inside New, we recreate the same structure here
 * by rendering <SubscriptionPlanNew /> is not correct. We'll fetch plan and pass to
 * the same PlanEditor definition that lives in this file.
 *
 * For clarity, we copy the minimal PlanEditor & ImagePicker again (they're tiny),
...
/* ------------------------------------------------------------------ */
/* Plan Editor (local copy for Edit page)                              */
/* ------------------------------------------------------------------ */
...
export default function SubscriptionPlanEdit() {
  ...
  React.useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        // Try a direct endpoint first (if you later add GET /plans/:id)
        let p = null;
        try {
          const direct = await api.get(`/subscription/plans/${id}`);
          p = direct.data;
        } catch {
  ...
  async function handleSave(payload) {
    try {
      setSaving(true);
      await api.put(`/subscription/plans/${id}`, payload);
      navigate("/subscriptions", { replace: true });
    } catch (e) {
  ...
  if (loading) return <div className="vd-muted">Loading…</div>;
  return <PlanEditor initial={plan} onSave={handleSave} saving={saving} />;
}
